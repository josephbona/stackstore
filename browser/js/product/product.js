...
app.controller('ProductController', function (AuthService, user, avgRating, $scope, product, CartService, ProductService, Session, $state, ReviewService) {
  ...
  $scope.addToCart = function(product){
    AuthService.getLoggedInUser()
      .then(function(user){
        ...
        if(!user){
          return CartService.addLineItem(product, $scope.quantity, user)
            .then(function(cart){
              $state.go('cart');
            })
            .catch(function(err){
